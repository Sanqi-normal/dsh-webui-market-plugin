...
window.__ModuleLoader__.load({ id: '@dsh-webui/dsh-webui-market-plugin', factory: (require) => {
  ...
  // Installed pnpm package name for a market plugin: match dependency keys and
  // values against the plugin's GitHub repo basename (covers specs pnpm writes,
  // e.g. github:owner/repo, git+https://...). Returns null when not installed.
  ...
  function isInstalled(plugin, installed) {
    return installedPkgName(plugin, installed) !== null
  }
...
`
  ...
  function MarketPanel() {
    ...
    const loadInstalled = () => {
      api('installed').then((r) => setData((d) => ({ ...d, installed: r }))).catch(() => setData((d) => ({ ...d, installed: null })))
    }
      ...
      const step = () => {
        ...
        api('op', { opId }).then((r) => {
          ...
          setOp((prev) => {
            ...
            if (o.status === 'running') {
              return { ...prev, phase: 'running', output: o.output, elapsedMs: o.elapsedMs }
            }
            return {
    ...
    const executeOp = () => {
      ...
      const params = op.kind === 'install'
        ? { source: op.target, profile: op.profile, binPath, label: op.label }
      ...
      api(op.kind === 'install' ? 'install' : 'uninstall', params).then((r) => {
        ...
        setOp({ ...op, phase: 'running', opId: r.opId, output: '', status: 'running', elapsedMs: 0 })
        pollOp(r.opId)
      }).catch((e) => {
    ...
    const envReady = envInfo && (envInfo.dshBin || binPath.trim()) && envInfo.node && envInfo.dshHome
    ...
    const modal = op && !op.minimized ? h('div', { className: 'mkts-modal-bg', onClick: () => { if (op.phase === 'running' || op.phase === 'starting') minimizeOp(); else closeOp() } },
      h('div', { className: 'mkts-modal', onClick: (e) => e.stopPropagation() },
        ...
        op.phase === 'confirm' ? h('div', { className: 'mkts-cmdrow' },
        ...
        op.phase === 'running' ? h('div', null,
          h('div', { className: 'mkts-cmdrow' },
            ...
            h('span', { style: { fontSize: 12 } },
              t('running') + ' · ' + fmt('elapsed', { s: Math.round((op.elapsedMs || 0) / 1000), t: 120 })),
    ...
    return h('div', { className: 'mkts' },
      envInfo ? h('div', { className: 'mkts-env' + (envReady ? '' : ' mkts-env-bad') },
        t('envLine') + ': DSH_HOME ' + (envInfo.dshHome ? '✓ ' + envInfo.dshHome : '✗') + ' · node ' + (envInfo.node ? '✓' : '✗') + ' · dsh ' + ((envInfo.dshBin || binPath.trim()) ? '✓' : '✗') +
        ((!envInfo.dshBin && !binPath.trim()) ? ' — dsh CLI 未定位' : ''),
      ) : null,
