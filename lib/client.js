...
window.__ModuleLoader__.load({ id: '@sanqi-normal/dsh-webui-market-plugin', factory: (require) => {
  ...
  // GitHub repo basenames are case-insensitive in practice while npm
  // dependency keys are lowercased, so compare both sides lowercased to
  // avoid missing installed plugins with mixed-case repo names (issue #1).
  ...
  const STR = {
    zh: {
      ...
      queueTitle: '任务队列', queueProgress: '共 {total} 项 · {active} 项进行中/排队', log: '日志', clearTask: '清除', cancelQueued: '取消排队',
    ...
    en: {
      ...
      queueTitle: 'Task queue', queueProgress: '{total} total · {active} active/queued', log: 'Log', clearTask: 'Clear', cancelQueued: 'Cancel',
...
.mkts-queue{border:1px solid var(--dsw-alias-border-l2);border-radius:10px;padding:8px 10px;margin-bottom:10px;background:var(--dsw-alias-bg-layer-2)}
  ...
  function MarketPanel() {
    ...
    const loadLocal = () => {
      api('installedAll', { profile: 'web' }).then((r) => {
        if (!r || !r.ok) {
          setNotice(String((r && (r.error || r.output)) || t('opFailed')))
          return
        }
    ...
    const localPlugins = local ? local.plugins.filter((p) => p.isBundle || p.inBundles) : []
    const localPlainCount = local ? local.plugins.length - localPlugins.length : 0
    ...
    const queuePanel = ops.length > 0 && queueOpen ? h('div', { className: 'mkts-queue' },
      h('div', { className: 'mkts-queue-head' },
        ...
        h('span', { style: { marginLeft: 'auto' } }, h('button', { className: 'mkts-cmdbtn', onClick: toggleQueue }, t('collapse'))),
      ),
      ops.map((o) => {
    ...
    return h('div', { className: 'mkts' },
      ...
      localOpen && local ? h('div', { className: 'mkts-local' },
        ...
        local.builtin && local.builtin.length > 0
          ...
          : null,
        localPlainCount > 0 ? h('div', { className: 'mkts-hint' }, t('localPlainDeps') + ' (' + localPlainCount + ')') : null,
      ) : null,
      modal,
      queuePanel,
