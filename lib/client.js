...
window.__ModuleLoader__.load({ id: '@sanqi-normal/dsh-webui-market-plugin', factory: (require) => {
  ...
  let LOCALE = 'en'
  try {
    const dl = (typeof document !== 'undefined' && document.documentElement && document.documentElement.lang) || ''
    const nl = String(dl || navigator.language || navigator.userLanguage || '')
    const low = nl.toLowerCase()
    if (low.startsWith('zh')) LOCALE = 'zh'
    else if (low.startsWith('ru')) LOCALE = 'ru'
  } catch (e) {}
  ...
  const STR = {
    zh: {
      ...
      notInstalled: '未安装', tabLabel: '插件市场',
    },
    en: {
      ...
      notInstalled: 'Not installed', tabLabel: 'Plugin Market',
    },
    ru: {
      search: 'Поиск плагинов…', all: 'Все', instFilter: 'Установленные', detail: 'Подробности', collapse: 'Свернуть',
      install: 'Установить', uninstall: 'Удалить', execute: 'Выполнить', cancel: 'Отмена', close: 'Закрыть',
      loading: 'Загрузка каталога плагинов…', noMatch: 'Подходящих плагинов нет',
      binPlaceholder: 'Путь к dsh CLI (укажите, если автоопределение не сработало; сохраняется)', binHint: 'dsh не найден: укажите apps/cli/lib/bin.js в корне репозитория dsh (из исходников — apps/cli/src/bin.ts) либо задайте DSH_BIN и перезапустите web', reprobe: 'Определить заново',
      installOk: 'Установлено — перезапустите веб-сервер для применения', uninstallOk: 'Удалено — перезапустите веб-сервер для применения', opFailed: 'Не удалось выполнить операцию',
      askDsh: 'Спросить DSH', askDshUnavailable: 'Не удалось открыть новый диалог: сервис сессий недоступен', askDshSent: 'Вопрос отправлен в новый диалог', askDshFailed: 'Не удалось отправить в новый диалог',
      hotOk: 'Установлено и подключено на лету — страница сейчас обновится',
      updateOk: 'Обновлено — перезапустите веб-сервер для применения', updateBtn: 'Обновить', updating: 'Обновление…', upToDate: 'Актуальная версия',
      updateFail: 'Не удалось проверить обновления', updLocal: 'локальная ссылка (dev)',
      running: 'Выполняется… (установка через pnpm может занять время)',
      cmdLabel: 'Команда установки (с сайта, включая целевой profile):', noCmd: '(официальной команды установки нет)',
      hint: 'После установки перезапустите веб-сервер. Для источников GitHub выполняется скрипт prepare из пакета (нужно разрешение pnpm allowBuilds). Установка в web проходит два уровня проверки: ① принимаются только источники из отобранного каталога; ② пробная установка — плагин ставится во временное окружение и запускается один раз; установка выполняется, только если запуск прошёл чисто (строка готовности dsh web), а при сбое показывается реальная ошибка запуска и ничего не изменяется. Простые плагины после установки подключаются на лету (без перезапуска). Чтобы установить принудительно, отметьте «Пропустить проверки безопасности» (на свой риск).',
      gh: 'GitHub ↗', envLine: 'Окружение', parseFail: 'Не удалось разобрать', fetchFail: 'Не удалось загрузить',
      submit: 'Отправка…', probing: 'Пробная установка… (плагин ставится во временное окружение и запускается один раз, чтобы убедиться, что web стартует; ~1–6 мин)', min: 'Свернуть в фон', kill: 'Прервать задачу', back: 'Назад',
      stDone: 'Готово', stFailed: 'Ошибка', stKilled: 'Прервано', stTimeout: 'Тайм-аут',
      stPending: 'В очереди', stChecking: 'Проверка', stBusy: 'Задача уже выполняется', stRefused: 'Отклонено', liveChip: 'Задача плагина',
      elapsed: 'Прошло {s} с (автопрерывание после {t} с)', newOp: 'Новая задача',
      site: 'Источник каталога плагинов',
      sortDefault: 'По умолчанию', sortHot: 'Популярные', sortNew: 'Новые',
      queueTitle: 'Очередь задач', queueProgress: 'Всего {total} · активных/в очереди {active}', log: 'Журнал', clearTask: 'Убрать', clearAll: 'Очистить всё', cancelQueued: 'Отменить', recentTasks: 'Недавние задачи',
      updateAll: 'Обновить все', noUpdatable: 'Нет плагинов для обновления',
      disable: 'Отключить', enable: 'Включить', disabledState: 'Отключён',
      localPlugins: 'Локальные плагины', localPluginsCount: 'Локальные плагины: {n}', localBuiltin: 'Встроенный', localCatalog: 'Из каталога', localExternal: 'Вне каталога',
      loadAll: 'Показать все', loadedCount: 'Показано {shown}/{total}',
      localPlainDeps: 'Обычные зависимости (не bundle) здесь не показаны', noLocal: 'Нет плагинов, управляемых зависимостями',
      kindGithub: 'GitHub', kindNpm: 'npm', kindLink: 'Локальная ссылка', kindFile: 'Локальный файл',
      syncTitle: 'Синхронизация между profile (web → остальные)', syncHint: 'В один клик: установить плагины, уже имеющиеся в web, в другой profile — только добавление, ничего не удаляется и не изменяется. Так как плагин уже установлен в web (локальная копия), белый список отобранного каталога не применяется; снимок состояния перед установкой по-прежнему создаётся. Для profile, отличных от web, пробная установка не выполняется; после синхронизации перезапустите соответствующее приложение.',
      syncMissing: 'к синхронизации: {n}', syncNone: 'Всё синхронизировано', syncTo: 'Синхронизировать в {name}',
      syncQueued: 'В очередь добавлено задач синхронизации: {n}', syncQueuedPartial: 'В очередь добавлено задач синхронизации: {n}; отклонено: {r} (повторите, отметив «Пропустить проверки безопасности» на карточке)', syncFailed: 'Не удалось синхронизировать: {e}',
      settingsTitle: 'Настройки установки', settingsHint: 'По умолчанию «устанавливать везде»: плагин ставится во все инициализированные profile на этой машине — есть web, значит в web; есть desktop, значит и в desktop. Если выключить, плагин будет установлен только в profile, выбранный при установке. Автосинхронизация — это локальное копирование (уже установленные источники проходят без белого списка каталога): только добавление, ничего не удаляется, для каждой цели создаётся снимок состояния перед установкой; для применения перезапустите каждое приложение.',
      autoSync: 'Автосинхронизация в другие profile после установки (включено по умолчанию)', foldHint: 'Подробнее',
      installTo: 'Установить в profile', skipWeb: 'Пропустить проверки безопасности (белый список источников + пробная установка; риск: web может перестать запускаться)', skipOther: 'Пропустить проверку по белому списку источников (риск: возможна установка из неучтённого источника)',
      notInstalled: 'Не установлен', tabLabel: 'Магазин плагинов',
    },
  }
...
`
    ...
    const askDsh = (o) => {
      ...
      const detail = [
        LOCALE === 'zh'
          ...
          : LOCALE === 'ru'
            ? 'При выполнении «' + kind + ' ' + target + '» в магазине плагинов dsh произошла ошибка. Помоги разобраться в причине и предложи решение.'
        ...
        '',
        LOCALE === 'zh' ? '错误日志：' : LOCALE === 'ru' ? 'Журнал ошибок:' : 'Error log:',
        '```',
        (o.output || (LOCALE === 'zh' ? '（无日志）' : LOCALE === 'ru' ? '(нет журнала)' : '(no log)')).slice(0, 12000) + ((o.output || '').length > 12000 ? '\n...[truncated]' : ''),
        '```',
      ].filter(Boolean).join('\n')
          ...
          return h('div', { key: p.url, className: 'mkts-item' },
            ...
            h('div', { className: 'mkts-actions' },
              h('span', { className: 'mkts-state ' + (inst ? (disabled ? 'mkts-state-off' : 'mkts-state-on') : 'mkts-state-off') },
                inst ? (disabled ? t('disabledState') : t('instFilter')) : t('notInstalled')),
  ...
  function apply(ctx) {
    ...
    slots.inject('settings.plugins.tab', () => slots.register(
      { name: 'settings.plugins.tab', id: 'market', order: 5, label: () => t('tabLabel') },
      MarketPanel,
    ))
  }
