...
window.__ModuleLoader__.load({ id: '@sanqi-normal/dsh-webui-market-plugin', factory: (require) => {
  ...
  const STR = {
    zh: {
      ...
      syncTitle: '跨 Profile 同步（web → 其它）', syncHint: '网页端（web）与桌面端（如 desktop）是各自独立的 profile：市场安装默认进 web，桌面应用不会自动加载这些插件（官网目录也只有 web 命令）。此面板把 web 里已装的插件一键补装到其它 profile——只新增缺失项，不删除、不改动任何现有内容。非 web profile 不跑试装验证（其组合由对应桌面壳定义），但仍过来源白名单 + 安装前快照；装好后重启对应应用生效。',
      syncMissing: '{n} 个待同步', syncNone: '已全部同步', syncTo: '同步到 {name}',
      syncQueued: '已加入队列 {n} 个同步任务', syncQueuedPartial: '已加入队列 {n} 个同步任务；{r} 个被白名单拒绝（可在卡片上勾选"跳过安全检查"重试）', syncFailed: '同步提交失败：{e}',
    ...
    en: {
      ...
      syncTitle: 'Sync across profiles (web → others)', syncHint: 'The web UI and a desktop shell (e.g. desktop) are separate profiles: the market installs into web by default and the desktop app never sees those plugins (the site catalog only ships web commands). This panel installs the plugins already present in web into another profile — additive only: it never removes or modifies anything already there. Non-web profiles skip the trial boot (their composition is defined by the shell), but still pass the source whitelist + pre-install snapshot; restart the target app after syncing.',
      syncMissing: '{n} to sync', syncNone: 'All synced', syncTo: 'Sync to {name}',
      syncQueued: '{n} sync tasks queued', syncQueuedPartial: '{n} sync tasks queued; {r} refused by the whitelist (retry with "skip safety checks" on the card)', syncFailed: 'Sync failed: {e}',
...
`
  ...
  function MarketPanel() {
    ...
    // has and the target lacks. Each install goes through the normal gated
    // pipeline (whitelist + pre-install snapshot + FIFO queue), so this is
    ...
    const runSync = (target) => {
      ...
      const chain = list.reduce((acc, item) => acc.then(() =>
        api('install', { source: item.source, profile: target, binPath, label: item.name })
          .then((r) => {
            if (r && r.ok) queued++
    ...
    return h('div', { className: 'mkts' },
      ...
      profiles && profiles.length > 1 ? h('div', { className: 'mkts-sync' },
        h('div', { className: 'mkts-sec' }, t('syncTitle')),
        h('div', { className: 'mkts-hint' }, t('syncHint')),
