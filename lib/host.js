...
function startOp(kind, profile, target, label, explicitBin) {
  ...
  op.child = child
  child.stdout.on('data', (d) => { op.output += d.toString() })
  child.stderr.on('data', (d) => { op.output += d.toString() })
  child.on('error', (err) => {
...
async function classifyPlugin(source) {
  ...
  try {
    ...
    if (!r.ok) return { known: false, fetchFailed: true }
    pkg = await r.json()
  } catch (e) {
    return { known: false, fetchFailed: true }
  }
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        if (method === 'installed') {
          const p = profileDir('web') + '/package.json'
          if (!existsSync(p)) return sendJson(res, 200, { ok: true, bundles: [], dependencies: {} })
        ...
        if (method === 'install' || method === 'uninstall') {
          ...
          return sendJson(res, 200, { ok: true, opId: started.opId })
        }
