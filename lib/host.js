...
function dshInvoke(explicit) {
  if (explicit && explicit.trim()) return { file: process.execPath, args: [explicit.trim()] }
  const entry = process.argv[1]
  if (entry !== undefined && /[\\/](?:bin\.(?:js|ts)|dsh)$/.test(entry)) {
    return { file: process.execPath, args: [...process.execArgv, entry] }
  }
  const cand = process.cwd().replace(/[\\/]+$/, '') + '/apps/cli/lib/bin.js'
  try {
    if (existsSync(cand)) return { file: process.execPath, args: [cand] }
  } catch {}
  if (process.env.DSH_BIN) return { file: process.execPath, args: [process.env.DSH_BIN] }
  return null
}
...
async function runProbe(explicitBin, source) {
  ...
  try {
    ...
    const install = await spawnCapture(inv.file,
      ...
      { cwd: profileDir, env, timeoutMs: PROBE_INSTALL_TIMEOUT })
    if (!install.ok) {
    ...
    const boot = await spawnCapture(inv.file,
      ...
      { cwd: home, env, timeoutMs: PROBE_BOOT_TIMEOUT, readyRe: READY_LINE_RE })
