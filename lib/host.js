...
async function runQueuedOp(op) {
  ...
  try {
    ...
    op.status = 'running'
    spawnOpChild(op)
    await new Promise((resolve) => { op.resolve = resolve })
  } finally {
    op.checkAbort = null
...
async function runProbe(explicitBin, source, signal) {
  ...
  try {
    ...
    const install = await spawnCapture(inv.file,
      ...
      { cwd: runCwd, env, timeoutMs: PROBE_INSTALL_TIMEOUT, signal })
    if (!install.ok) {
    ...
    const boot = await spawnCapture(inv.file,
      ...
      { cwd: runCwd, env, timeoutMs: PROBE_BOOT_TIMEOUT, readyRe: READY_LINE_RE, signal })
...
function spawnCapture(exe, args, { cwd, env, timeoutMs, readyRe, signal }) {
  return new Promise((resolve) => {
...
/** Same matching rule as the client's installedPkgName, host-side. */
function matchesCatalog(name, spec, plugin) {
...
function installedAll(profile) {
  ...
  const plugins = Object.keys(deps).map((name) => {
    ...
    return {
      ...
      inCatalog: catalog.some((p) => matchesCatalog(name, spec, p)),
    }
  })
...
export { classifyPlugin, runProbe, whitelistSource, loadCatalog, parseSimplePatch, checkUpdates, parseSite, registryToCatalog, setDisabledState, installedAll } // test hooks; cordis only reads name/inject/apply
    ...
    handler: async (req, res) => {
      try {
        ...
        if (method === 'installed') {
          ...
          const p = profileDir(profile) + '/package.json'
          if (!existsSync(p)) return sendJson(res, 200, { ok: true, profile, bundles: [], dependencies: {}, disabled: [] })
