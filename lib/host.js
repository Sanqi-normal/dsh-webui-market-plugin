...
function dshInvoke(explicit) {
  if (explicit && explicit.trim()) {
    const entry = explicit.trim()
    return { file: process.execPath, args: tsxArgs(entry), cwd: undefined }
  }
  const entry = process.argv[1]
  if (entry !== undefined && /[\\/](?:bin\.(?:js|ts)|dsh)$/.test(entry)) {
    return { file: process.execPath, args: [...process.execArgv, ...tsxArgs(entry)], cwd: dirname(entry) }
  }
  ...
  if (process.env.DSH_BIN) {
    const entry = process.env.DSH_BIN
    return { file: process.execPath, args: tsxArgs(entry), cwd: undefined }
  }
  return null
}
...
/**
 * A `.ts` CLI entry needs the tsx loader. When the host already runs under
 * tsx (execArgv carries `--import tsx`) that is covered; otherwise prepend it
 * so a bare `node bin.ts` still works. tsx resolves from the spawn cwd, which
 * dshInvoke pins to the entry's directory for source launches.
 * @returns {string[]} extra argv before the entry.
 */
function tsxArgs(entry) {
  if (!/\.ts$/.test(entry)) return [entry]
  const hasTsx = process.execArgv.some((a) => String(a).includes('tsx'))
  return hasTsx ? [entry] : ['--import', 'tsx', entry]
}
...
/**
 * Load the plugin catalog: live site first, bundled snapshot as offline
 * fallback (mirroring dsh-market's registry + snapshot), cached briefly.
 * @returns {Promise<{plugins: any[], cats: any[], source: 'live'|'snapshot'|'none'}>}
 */
async function loadCatalog() {
  ...
  try {
    const url = 'https://awesome-dsh-plugin.com/zh/'
    const r = await fetch(url, { redirect: 'follow', signal: AbortSignal.timeout(10000) })
    if (!r.ok) throw new Error('HTTP ' + r.status)
    const parsed = parseSite(await r.text())
    if (!Array.isArray(parsed.plugins) || parsed.plugins.length === 0) throw new Error('empty catalog')
    catalogCache = { at: now, data: parsed }
...
async function hotMount(ctx, profile, packageName) {
  try {
    ...
    const handle = ctx.plugin(HotTree, { path: fileURLToPath(pathToFileURL(file)) })
    await handle.await()
...
export { classifyPlugin, runProbe, whitelistSource, loadCatalog, parseSimplePatch, checkUpdates } // test hooks; cordis only reads name/inject/apply
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        if (method === 'list') {
          // Live site with bundled snapshot fallback (offline browsing).
        ...
        if (method === 'install' || method === 'uninstall') {
          ...
          const label = String(body.label || target)
          // Uninstall: dispose the live hot mount FIRST. `dsh plugin remove`
          // deletes the package from node_modules but a hot-mounted Include
          // subtree would stay in the running composition — client-modules
          // would keep serving a bundle that no longer exists and the next
          // page reload would fail to import it.
          if (method === 'uninstall') {
